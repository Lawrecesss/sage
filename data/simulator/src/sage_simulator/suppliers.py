...
`Supplier`'s shape lives in `sage_models` (shared with every other service); this
module only builds instances of it. `build_suppliers(config)` returns a
deterministic `tuple[Supplier, ...]` of length
