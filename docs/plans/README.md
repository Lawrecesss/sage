...
| **M2** | [m2-agents.md](m2-agents.md) | MCP tool server, OpenClaw agent config + prompts, eval harness |
| **M3** | [m3-frontend.md](m3-frontend.md) | The Next.js app, design system, Brief / Signals / Ask / Connections |
| **M4** | [m4-platform.md](m4-platform.md) | Lightsail deploy, API, `mcp`/`openclaw` containers, CI/CD — **plus** pitch, deck, demo video |
...
| **Sep 21** | **A real anomaly, detected and explained, visible in a browser on the deployed instance.** The most important gate in the plan — if it's at risk, everyone stops adding scope and makes this work | M1+M2+M3+M4 |
| **Sep 28 (hard)** | The hero scenario (supplier delay → stockout → revenue + margin → recommended recovery action) runs end to end, unassisted, fired by an OpenClaw automation. **Code freeze. `git tag demo-freeze`** | all |
...
1. **LLM endpoint creds — M4, today.** Get `LLM_BASE_URL` + `LLM_API_KEY` from the
   ...
   Also today: **spike whether that endpoint registers under OpenClaw's
   `models.providers`** (see `docs/decisions/0003-openclaw-agent-runtime.md`) — M2 is
   blocked on this. Also today: create the Lightsail instance + static IP + DNS.
...
3. **Contract freezes by Sep 14:**
   - Tool schemas (now MCP) — **M1 ↔ M2** — [`../contracts/tool-schemas.md`](../contracts/tool-schemas.md)
...
| From → To | Artifact | By |
| --- | --- | --- |
| M1 → M2 | Tool schemas frozen — MCP tools (`query_metric`, `list_metrics`, `get_signals`, `compare_period`, `trace_lineage`) | Sep 14 |
...
| M4 → all | First full deploy on the instance (db · mcp · openclaw · api · web · caddy) | Sep 21 |
...
| **M2** | Python, prompting, eval/measurement instinct, and comfort configuring a gateway rather than writing an agent loop by hand | Blocked until M4 hands over the LLM endpoint creds; `sage-briefing`'s correlation is the product |
