...
| **M2** | [m2-agents.md](m2-agents.md) | Strands agents, tool surface, prompts, Bedrock, eval harness |
| **M3** | [m3-frontend.md](m3-frontend.md) | The Next.js app, design system, Brief / Signals / Ask / Connections |
| **M4** | [m4-platform.md](m4-platform.md) | CDK, API, Telegram, CI/CD, cost — **plus** pitch, deck, demo video |
...
| S2 | Sep 15–21 | The vertical slice, end to end on AWS |
...
| **Sep 21** | **A real anomaly, detected and explained, visible in a browser on AWS and on a phone.** The most important gate in the plan — if it's at risk, everyone stops adding scope and makes this work | M1+M2+M3+M4 |
| **Sep 28 (hard)** | The hero scenario (supplier delay → stockout → revenue + margin → recommended recovery action) runs end to end, unassisted, from a scheduled trigger. **Code freeze. `git tag demo-freeze`** | all |

## Critical path — start these first

1. **Bedrock model access request — M4, today.** Approval can take days and blocks
   every M2 sprint. Until it lands, M2 develops against the direct Anthropic API
   (Strands abstracts the provider — it's a config flip).
2. **API `/health` + app skeleton — M4, Sprint 1.** M3 can build the static Brief
   without it, but cannot wire the *live* Brief in Sprint 2 until the API answers.
...
| M4 → M3 | API base URL + `/health` + OpenAPI docs reachable | Sep 15 |
...
| M4 → all | First full AWS deploy (API + web + worker) | Sep 21 |
...
| **M2** | Python, LLM prompting, eval/measurement instinct | Blocked on Bedrock access; the Correlator is the product |
| **M3** | React / TypeScript / Next.js, visual taste | Least blocked (static mock in S1); the Brief screenshot carries the pitch |
| **M4** | AWS, IaC (CDK), CI/CD, cost discipline — **and comfort presenting** | Also owns pitch + deck + demo video, so it suits whoever will be on stage |
...
twice-weekly integration (Tue + Fri), end to end on AWS from the end of Sprint 1.
