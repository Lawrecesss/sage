...
- **`packages/agents` → `packages/mcp` (`sage_mcp`).** It stops being an agent
...
- **Agent definitions live in OpenClaw config, not Python.** A new top-level
  `openclaw/` directory holds the config template (`agents.entries`, MCP server
...
- **Channel delivery is unresolved and out of scope for this change.**
  `packages/notifier` (Telegram) is left exactly as it is, unwired. Whether it
...
chain is now config + prompts in `openclaw/`, executed by a runtime this repo
