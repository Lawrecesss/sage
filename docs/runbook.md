...
./scripts/dev.sh         # sage-mcp  &  pnpm --filter web dev
...
| Var | Used by | Note |
| --- | --- | --- |
| `DATABASE_URL` | warehouse, detectors, mcp | `postgresql+psycopg://sage:sage@localhost:5432/sage` locally; `@db:5432` in compose |
...
| `OPENCLAW_BASE_URL` / `OPENCLAW_TOKEN` | evals, backend (future) | internal gateway URL + full-operator bearer token |
| `OPENCLAW_AGENT_BRIEFING` / `OPENCLAW_AGENT_ASK` | backend (future) | agent ids OpenClaw exposes |
| `MCP_URL` | openclaw (container) | where OpenClaw finds the `sage` MCP server |
| `SAGE_DOMAIN` | caddy | public hostname |
...
ssh ubuntu@<ip> 'cd /opt/sage && docker compose ... exec mcp sage-warehouse init-db && ./scripts/seed-demo.sh'
```
