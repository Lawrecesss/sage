...
- **Tool surface:** the `sage` MCP server (`packages/mcp`, container `mcp`) is
...
- **Entry point:** `packages/api` calls this gateway's OpenAI-compatible
...
2. Add an entry under `agents.entries` in `openclaw.json5` — model, system
   ...
   more).
3. Point `sage_shared.settings` (and whichever `packages/api` router needs it)
   at the new agent id.
