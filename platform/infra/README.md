...
DNS A record → Lightsail static IP → the instance
  ...
  ├─ web       next start (standalone build) — the only public entry point; will
  │            serve /api/* itself once the Next.js backend lands (docs/team-plan.md)
  └─ caddy     :80/:443 — TLS + reverse proxy ({$SAGE_DOMAIN} → web)
...
| `docker-compose.prod.yml` | Override layered on the repo-root `docker-compose.yml` — adds `web`, `caddy` (`mcp` + `openclaw` come from the base file's `agent` profile) |
