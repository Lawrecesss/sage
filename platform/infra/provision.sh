...
#
#   bash platform/infra/provision.sh create     # make the instance + static IP + firewall
#   bash platform/infra/provision.sh setup      # ssh in, install docker, clone, first build
#   bash platform/infra/provision.sh restart    # git pull + compose up -d --build
#   bash platform/infra/provision.sh logs       # tail compose logs
set -euo pipefail
...
COMPOSE="docker compose --profile agent -f docker-compose.yml -f platform/infra/docker-compose.prod.yml"
...
case "${1:-}" in
  create)
    aws lightsail create-instances --region "$REGION" \
      ...
      --user-data "$(cat platform/infra/cloud-init.yaml)"
    ...
    echo "static IP: $(ip)  — point your DNS A record here, then: bash platform/infra/provision.sh setup"
    ;;
  setup|restart)
    HOST="$(ip)"
