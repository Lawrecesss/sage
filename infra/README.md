...
DNS A record → Lightsail static IP → the instance
  docker compose --profile agent -f docker-compose.yml -f infra/docker-compose.prod.yml up -d
  ...
  ├─ openclaw  agent gateway; config seeded from ./openclaw (prompts, agents, cron)
  ...
  daily brief → an OpenClaw automation (openclaw/automations/daily-brief.md),
...
#    see openclaw/automations/daily-brief.md
```
